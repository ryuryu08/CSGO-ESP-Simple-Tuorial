...
func ReadMemoryUint32(handle syscall.Handle, address uint) (uint32, error) {
	var data [4]byte
...
func ReadMemoryViewMatrix(handle syscall.Handle, address uint) (matrix [4][4]float32, err error) {
	var data [64]byte
	_, err = ReadProcessMemory(handle, uintptr(address), data[:])
	if err != nil {
		return
	}
	array := helper.Float32SliceFromBytes(data[:])
...
func ReadMemoryFloat32(handle syscall.Handle, address uint) (float32, error) {
	var data [4]byte
	_, err := ReadProcessMemory(handle, uintptr(address), data[:])
	if err != nil {
		return 0, err
...
func ReadMemoryFloat64(handle syscall.Handle, address uint) (float64, error) {
	var data [8]byte
	_, err := ReadProcessMemory(handle, uintptr(address), data[:])
	if err != nil {
		return 0, err
